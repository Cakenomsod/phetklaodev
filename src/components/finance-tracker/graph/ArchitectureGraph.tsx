...
import NodeDetailPanel from "@/src/components/finance-tracker/graph/NodeDetailPanel";
...
export default function ArchitectureGraph({ className }: ArchitectureGraphProps) {
  const [hoveredId, setHoveredId] = useState<ArchNodeId | null>(null);
  const [selectedId, setSelectedId] = useState<ArchNodeId | null>(null);
  const reduceMotion = useReducedMotion();

  const activeId = selectedId ?? hoveredId;
  const highlightedNodes = useMemo(
    () => (activeId ? getConnectedNodeIds(activeId) : new Set<ArchNodeId>()),
    [activeId],
  );

  const selectedNode = selectedId ? getNodeById(selectedId) ?? null : null;

  function isEdgeActive(from: ArchNodeId, to: ArchNodeId) {
  ...
  function isNodeDimmed(id: ArchNodeId) {
    if (!activeId) return false;
  ...
  return (
    <div className={cn("relative w-full", className)}>
      <div className="relative aspect-[10/7] w-full overflow-hidden rounded-2xl border border-border-default bg-bg-secondary/50">
        <svg
        ...
        >
          <defs>
            <linearGradient id="ft-edge-gradient" x1="0%" y1="0%" x2="100%" y2="0%">
            ...
            return (
              <g key={edge.id}>
                ...
                <motion.path
                  d={path}
                  fill="none"
                  stroke={active ? "url(#ft-edge-gradient)" : "var(--theme-graph-edge)"}
                  ...
                  animate={
                    reduceMotion
                      ? {}
                      : active
                        ? { strokeDashoffset: [24, 0], opacity: 1 }
      ...
      <p className="mt-4 text-center font-mono text-[10px] tracking-wider text-text-muted uppercase">
        Hover to trace data flow · Click a node for details
      </p>
    </div>
  );
}
