        ...
        <>
          ...
          <dl className="mt-8 grid gap-4 sm:grid-cols-2">
            <div 
              ...
              tabIndex={0}
              onClick={() => {
                const urls = gpaxAssetIds.map(id => `/api/proxy-image?assetId=${encodeURIComponent(id)}`);
            ...
            <div
              ...
              role="button"
              tabIndex={0}
              onClick={() => setActivePreview(`/api/proxy-image?assetId=${encodeURIComponent(ieltsAssetId)}`)}
              onKeyDown={(e) => {
                if (e.key === "Enter" || e.key === " ") {
                  setActivePreview(`/api/proxy-image?assetId=${encodeURIComponent(ieltsAssetId)}`);
                }
              }}
            >
              <dt className="text-xs font-semibold tracking-wide text-[var(--portfolio-muted)] uppercase">
